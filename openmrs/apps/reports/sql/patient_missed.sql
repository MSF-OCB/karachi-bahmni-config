SELECT 
    ...
    Status
FROM
    openmrs.patient_appointment pa
        INNER JOIN
    `openmrs`.`person` AS PR ON PR.PERSON_ID = pa.PATIENT_ID
        AND PR.voided = 0
        INNER JOIN
    `openmrs`.`person_name` AS NM ON NM.PERSON_ID = pa.PATIENT_ID
        AND NM.voided = 0
        AND preferred = 1
        INNER JOIN
    `openmrs`.`patient_identifier` AS ID ON ID.PATIENT_ID = pa.PATIENT_ID
        AND ID.IDENTIFIER_TYPE = 3
        AND ID.voided = 0
        AND ID.preferred = 1
        LEFT JOIN
    `openmrs`.`patient_identifier` AS ID2 ON ID2.PATIENT_ID = pa.PATIENT_ID
        AND ID2.IDENTIFIER_TYPE = 4
        AND ID2.voided = 0
        LEFT JOIN
    openmrs.person_attribute PA2 ON PA2.PERSON_ID = pa.PATIENT_ID
